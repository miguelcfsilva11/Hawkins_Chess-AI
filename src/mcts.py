...
class mcts:
    def search(self, mx, player, last_move, castling_chance):
        ...
        while time.time() - starting_point <= 15:
            cicles += 1
    ...
    def rollout(self, leaf, last_move, castling_chance, depth):
        ...
        abroll_score = mcts.minimax(self, mx, 3, -1*10**6, 1*10**6, False, castling_chance, last_move)
        print(abroll_score)
        score += abroll_score
        return abroll_score
        """
        ...
        if maximizing_player:
            ...
            for state in possible_states:
                ...
                if state in transposition_table.keys():
                    ...
                    eval = mcts.minimax(self, state, depth-1, alpha, beta, False, castling_chance, last_move)
            ...
            for state in possible_states:
                if state in transposition_table.keys():
                    ...
                    eval = mcts.minimax(self, state, depth-1, alpha, beta, True, castling_chance, last_move)
                    transposition_table[state] = eval
                if eval < min_eval:
                    min_eval = eval
                beta = min(beta, eval)
