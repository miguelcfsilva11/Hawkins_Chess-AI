	...
	def evaluate(self, mx):
		...
		for pos in range(len(mx)):
			...
			if mx[pos].lower() == mx[pos]:
				score += (piece_to_table[mx[pos]][pos]/5)
			else:
				score -= (piece_to_table[mx[pos]][pos]/5)
		if minor_black_pieces <= 2 or minor_white_pieces <= 2: 
			score += (kingend_table[::-1][black_king_spot] - kingend_table[white_king_spot])/5
			score += math.sqrt(abs(4 - white_king_spot//8)^2 + abs(4 - white_king_spot%8)^2)
			score += (5 - math.sqrt(abs(black_king_spot//8 - white_king_spot//8)^2 + abs(black_king_spot%8 - white_king_spot%8)^2))
		else:
			score += (kingmid_table[::-1][black_king_spot] - kingmid_table[white_king_spot])/5
